...
module.exports = async function handler(req, res) {
  ...
  if (!apiKey) {
    return res.status(503).json({ ok: false, error: 'Voice provider is not configured yet.' });
  }

  const { text, voice = 'Adam — English · Deep / Firm' } = req.body || {};
  ...
  if (!cleanText || cleanText.length > MAX_TEXT_LENGTH) {
    return res.status(400).json({ ok: false, error: `Text must be between 1 and ${MAX_TEXT_LENGTH} characters.` });
  }

  const voiceId = getVoiceId(voice);
  if (!voiceId) {
    return res.status(400).json({ ok: false, error: 'Selected voice is not configured.' });
  }
